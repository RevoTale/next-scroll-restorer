...
export const getScrollTimestamp = (state: HistoryState): number | null =>
	retrieveNum('memo_timestamp', state);
export const getPopstateTimestamp = (state: HistoryState): number | null =>
	retrieveNum('popstate_timestamp', state);
export const getIsNavigatingHistory = (state: HistoryState): boolean =>
	state === null ? false : Boolean(state[getKey('is_navigating_history')]);
